...
#define SELECT_RANDOM_MESSAGES_START     "SELECT time, nick, message FROM messages WHERE "
#define SELECT_RANDOM_MESSAGES_CONDITION "id=(SELECT ceil((SELECT case WHEN x < 0 THEN -1 * x else x end FROM (select random() as x)) % (SELECT max(id) FROM messages)))"
